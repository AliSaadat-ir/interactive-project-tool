...
async function exportProject() {
  ...
  const { getExportFilename } = require('../utils/date');
  const outputFile = path.join(process.cwd(), getExportFilename('export'));
  ...
  if (result.success) {
    ...
    if (openFile) {
      ...
      if (process.platform === 'win32') {
        // Windows: Use notepad to open text file
        exec(`notepad "${outputFile.replace(/\//g, '\\')}"`);
      } else if (process.platform === 'darwin') {
        // macOS: Use default text editor
        exec(`open "${outputFile}"`);
      } else {
        // Linux: Try common text editors
        exec(`xdg-open "${outputFile}" || gedit "${outputFile}" || nano "${outputFile}"`);
      }
    }
  } else {
...
async function performExport(sourceDir, outputFile, useGitignore = false) {
  try {
    ...
    output.push(`// Exported at: ${formatDateTimeForDisplay()}`);
