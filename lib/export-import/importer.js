    ...
    switch (choice.value) {
      ...
      case 'create_new':
        ...
        if (fileExists(newPath)) {
          ...
          const useExisting = await askConfirm('Use existing folder?');
          if (useExisting) {
...
async function importProject() {
  ...
  console.log();
  print(`📄 Source file: ${path.basename(inputFile)}`, 'white');
  print(`📁 Destination: ${destination.path}`, 'white');
  if (destination.create) {
    print(`🆕 New folder will be created`, 'green');
  }
  console.log();

  const confirm = await askConfirm('Start import with these settings?');
