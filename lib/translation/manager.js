...
`;
  ...
  async generateReport() {
    ...
    const analysis = await this.analyzer.analyze(this.lastScanResult.usedKeys, allKeys);
    const reportPaths = await this.reporter.generateReport(analysis, this.projectRoot, this.lastScanResult);
    
    // Auto-open reports if enabled
    if (this.settings.autoOpenReports && this.openFileCallback) {
      console.log();
      const openReports = await askConfirmMenu('Open generated reports?', true);
      
      if (openReports) {
        // Open markdown report
        await this.openFileCallback(reportPaths.markdown, this.settings);
        
        // Ask if user wants to open JSON report too
