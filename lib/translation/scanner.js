...
class TranslationScanner {
  constructor(projectRoot) {
    ...
    this.fallbackTexts = {};
    this.fallbackConflicts = {}; // key -> array of different fallbacks
  ...
  async scanProject() {
    ...
    // Resolve fallback conflicts if any
    if (Object.keys(this.fallbackConflicts).length > 0) {
      await this.resolveFallbackConflicts();
    }
  ...
  async resolveFallbackConflicts() {
    print('\n⚠️  Found translation keys with different fallback texts', 'yellow');
    print('Let\'s resolve these conflicts:\n', 'dim');
    
    for (const [key, fallbacks] of Object.entries(this.fallbackConflicts)) {
      print(`📝 Key: "${key}"`, 'cyan');
      print('Found different fallback texts:', 'white');
      
      // Create unique options
      const uniqueFallbacks = [...new Set(fallbacks)];
      const options = uniqueFallbacks.map((text, index) => ({
        name: `"${text}"`,
        value: text
      }));
      ...
      if (selected.value === 'custom') {
        ...
        this.fallbackTexts[key] = customText.trim() || uniqueFallbacks[0];
      } else {
          ...
          if (i < 7 && match[2]) {
            ...
            if (this.fallbackTexts[key] && this.fallbackTexts[key] !== match[2]) {
              ...
              if (!this.fallbackConflicts[key]) {
                this.fallbackConflicts[key] = [this.fallbackTexts[key]];
              }
              if (!this.fallbackConflicts[key].includes(match[2])) {
                this.fallbackConflicts[key].push(match[2]);
              }
            } else {
