...
class TranslationScanner {
  ...
  // Normalize quotes in text (convert all to double quotes for comparison)
  ...
  async resolveFallbackConflicts() {
    ...
    for (let i = 0; i < conflictKeys.length; i++) {
      ...
      // Group by normalized text content (ignoring quote types)
      const textGroups = {};
      ...
      Object.entries(textGroups).forEach(([content, group]) => {
        ...
        // Create display text showing all variations
        let displayText = content;
        if (group.variations.length > 1) {
          const variationsList = group.variations.map(v => v).join(', ');
          displayText = `${content} (variations: ${variationsList})`;
        } else {
      ...
      for (let i = 0; i < patterns.length; i++) {
        ...
        for (const match of matches) {
          ...
          if (i < 7 && match[2]) {
            ...
            const fallbackContent = match[3]; // This is just the content
            
            location.fallback = fullFallbackMatch;
            
            // Check for conflicts based on content only
            if (this.fallbackTexts[key]) {
              // Compare content only (not quotes)
              if (this.fallbackTexts[key] !== fallbackContent) {
                ...
                if (!this.fallbackConflicts[key]) {
                  ...
                  const originalLocations = this.keyUsageMap.get(key)
                    ?.filter(loc => {
                      if (loc.fallback) {
                        const originalContent = this.extractTextContent(loc.fallback);
                        return originalContent === this.fallbackTexts[key];
                      }
                      return false;
                    }) || [];
                ...
                let existingConflict = this.fallbackConflicts[key]
                  .find(c => this.extractTextContent(c.text) === fallbackContent);
